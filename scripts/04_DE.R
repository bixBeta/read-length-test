...
png(filename = "figures/pheatmap.TOP.200.variableGenes.png", width = 4000, height = 20000, res = 100)
pheatmap::pheatmap(avg.scaled.matrix.all.genes[rownames(avg.scaled.matrix.all.genes) %in% top200,], scale = "none", 
...
case.D1.v.control.D1 = list()

for (i in 1:length(levels(Idents(cca.obj)))) {
  case.D1.v.control.D1[[i]] <- FindMarkers(cca.obj, ident.1 = "case.D1", 
                                                       ident.2 = "control.D1", group.by = "condition",
                                                       subset.ident = levels(Idents(cca.obj))[i], 
                                                       only.pos = F, logfc.threshold = 0.1)
  
  names(case.D1.v.control.D1)[[i]] <- paste0("CLUSTER__", levels(Idents(cca.obj))[i])
  
}

save(long.case.D1.v.long.control.D1, short.case.D1.v.short.control.D1, case.D1.v.control.D1, file = "data/long.and.short.case.v.ctrl.D1.Rdata")
